      ...
      describe("vehnt tests - " + name, () => {
        ...
        describe("with delegated vehnt", () => {
          ...
          describe("backstop floor delivery", () => {
            ...
            it("still lands on the target when an HST cut is taken", async () => {
              // issue_rewards_v0 takes the HST cut off the DAO-level mint before the
              // sub-DAO split, so the baseline the top-up is sized against has to read the
              // schedule. At 30% HST the emission reaching deployers drops by 30% and the
              // top-up has to grow by the same amount for delivery to stay on target.
              await program.methods
                .updateDaoV0({
                  authority: null,
                  emissionSchedule: null,
                  hstEmissionSchedule: [
                    { startUnixTime: new BN(0), percent: 30 },
                  ],
                  hstPool: null,
                  netEmissionsCap: new BN(0),
                  proposalNamespace: null,
              ...
              // What the emission alone delivers to deployers at 30% HST: the DAO-level
              // mint less the HST cut, times the sub-DAO share, less the 6% delegation
              // slice. Tolerance is a fraction of that baseline rather than of the target,
              // so it does not slacken as the cloned mainnet price moves: a baseline that
              // skipped the HST cut measures the shortfall against a figure 1/0.7 too
              // large, so delivery lands 43% of this baseline short of the target.
              const baseline =
                EPOCH_REWARDS * (mobileShare / 0xffffffff) * 0.7 * 0.94;
              expect(topUp + baseline).to.be.closeTo(target, baseline * 0.05);
            });
          });
