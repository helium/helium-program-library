...
import { ensureDCIdl, ensureHEMIdl, ensureHSDIdl, initWorld, ensureTuktukDcaIdl } from "./utils/fixtures";
import { createDcaServer, runAllTasks as runAllTasksUtil } from "./utils/dca-test-server";
...
describe("dc-auto-topoff", () => {
  ...
  beforeEach(async () => {
    ...
    await createAtaAndTransfer(provider, hntMint, new anchor.BN(1000_00000000), me, swapPayer) // 1000 HNT
  })
  ...
  it("should initialize an auto topoff", async () => {
    ...
    expect(Buffer.from(autoTopOffAcc.schedule).toString("utf-8").replace(/\0/g, '')).to.equal("0 0 * * * *")
  ...
  describe("with an auto topoff", () => {
    ...
    beforeEach(async () => {
      ...
      const [nextTask, nextHntTask] = nextAvailableTaskIds(taskQueueAcc.taskBitmap, 2, false)
    ...
    it("should allow updating schedule", async () => {
      const taskQueueAcc = await tuktukProgram.account.taskQueueV0.fetch(taskQueue)
      const existingAccount = await program.account.autoTopOffV0.fetch(autoTopOff)
      const taskRentRefund = (await tuktukProgram.account.taskV0.fetch(existingAccount.nextTask)).rentRefund
      const hntTaskRentRefund = (await tuktukProgram.account.taskV0.fetch(existingAccount.nextHntTask)).rentRefund
      const [nextTask, nextHntTask] = nextAvailableTaskIds(taskQueueAcc.taskBitmap, 2, false)
      ...
      expect(autoTopOffAcc.nextHntTask.toBase58()).to.equal(taskKey(taskQueue, nextHntTask)[0].toBase58())
    })
    ...
    it("should trigger DCA when HNT is below threshold", async () => {
      ...
      const inputPriceUpdate = await pythReceiver.receiver.account.priceUpdateV2.fetch(USDC_PRICE_FEED)
      const outputPriceUpdate = await pythReceiver.receiver.account.priceUpdateV2.fetch(HNT_PRICE_FEED)

      console.log(`Input (USDC) Price: ${inputPriceUpdate.priceMessage.price.toString()} (expo: ${inputPriceUpdate.priceMessage.exponent})`)
      console.log(`Output (HNT) Price: ${outputPriceUpdate.priceMessage.price.toString()} (expo: ${outputPriceUpdate.priceMessage.exponent})`)
      ...
      console.log(`Expected HNT per swap: ${hntPerSwap.toString()} (${hntPerSwap.toNumber() / 100000000} HNT)`)

      // Update the existing auto topoff with the specific DCA parameters for this test
      const taskQueueAcc2 = await tuktukProgram.account.taskQueueV0.fetch(taskQueue)
      const existingAccount = await program.account.autoTopOffV0.fetch(autoTopOff)
      const taskRentRefund = (await tuktukProgram.account.taskV0.fetch(existingAccount.nextTask)).rentRefund
      const hntTaskRentRefund = (await tuktukProgram.account.taskV0.fetch(existingAccount.nextHntTask)).rentRefund
      const [nextHntTask2, nextTask2] = nextAvailableTaskIds(taskQueueAcc2.taskBitmap, 2, false)
      ...
      await createAtaAndTransfer(provider, hntMint, startingHnt.toNumber(), me, autoTopOff)

      // Fund with enough DCA mint tokens for exactly 2 swaps
      const totalDcaMintNeeded = dcaSwapAmount.muln(numOrders)
      await createAtaAndTransfer(provider, dcaMint, totalDcaMintNeeded.toNumber(), me, autoTopOff)

      console.log("Waiting for scheduled topoff with DCA...")
      await new Promise(resolve => setTimeout(resolve, 4000))

      const hntAccountBefore = await getAccount(provider.connection, getAssociatedTokenAddressSync(hntMint, autoTopOff, true))
      console.log(`HNT balance before topoff: ${hntAccountBefore.amount}`)
      expect(hntAccountBefore.amount.toString()).to.equal(startingHnt.toString(), "Starting HNT balance should be exactly 10 HNT")
      ...
      const hntAccountFinal = await getAccount(provider.connection, getAssociatedTokenAddressSync(hntMint, autoTopOff, true))
      console.log(`HNT balance after second swap: ${hntAccountFinal.amount} (expected: ${hntThreshold.toString()})`)
      ...
      expect(finalAmount.gte(hntThreshold.sub(new anchor.BN(2)))).to.equal(true, "After two swaps, HNT should be at or above threshold (allowing for 1 bone rounding)")

      // Verify DCA mint tokens were fully consumed
      const dcaMintAccountFinal = await getAccount(provider.connection, getAssociatedTokenAddressSync(dcaMint, autoTopOff, true))
      console.log(`DCA mint balance after swaps: ${dcaMintAccountFinal.amount}`)
      expect(dcaMintAccountFinal.amount.toString()).to.equal("0", "All DCA mint tokens should be consumed")
    })
