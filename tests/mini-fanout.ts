...
describe("mini-fanout", () => {
  ...
  let shares: {
    wallet: PublicKey;
    share: { share: { amount: number } } | { fixed: { amount: anchor.BN } };
  }[] = [];
  ...
  it("should initialize a fanout", async () => {
    ...
    for (let i = 0; i < shares.length; i++) {
      expect(miniFanoutAcc.shares[i].wallet.toBase58()).to.equal(
        shares[i].wallet.toBase58()
      );
      if ("share" in shares[i].share) {
        // @ts-ignore
        expect(miniFanoutAcc.shares[i].share.share!.amount).to.equal(
          shares[i].share.share!.amount
        );
      } else {
        // @ts-ignore
        expect(miniFanoutAcc.shares[i].share.fixed!.amount.toString()).to.equal(
          shares[i].share.fixed!.amount.toString()
        );
      }
  ...
  describe("pre task shapes", () => {
    ...
    it("queues a remote pre task the oracle signs", async () => {
      ...
      expect(acc.preTask!.remoteV0!.signer.toBase58()).to.equal(
        ORACLE_SIGNER.toBase58()
      );
          ...
          compiledV0: [
            {
              ...(memoPreTask as any),
              signerSeeds: [
                [Buffer.from("helium", "utf-8"), Buffer.from([253])],
              ],
            },
          ],
        })
  ...
  describe("with a fanout", () => {
    ...
    it("should allow updating wallet delegates", async () => {
      ...
      const miniFanoutAcc = await program.account.miniFanoutV0.fetch(fanout);
      expect(miniFanoutAcc.shares[0].delegate.toBase58()).to.equal(
        newWallet.publicKey.toBase58()
      );
    });
