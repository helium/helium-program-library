...
use anchor_spl::token::{TokenAccount, Mint, Transfer, self, Token};

use crate::{FanoutV0, FanoutVoucherV0, voucher_seeds};
...
pub fn handler(ctx: Context<DistributeV0>) -> Result<()> {
  ...
  let inflow = curr_balance.checked_sub(ctx.accounts.fanout.last_snapshot_amount).unwrap();
  let tss = ctx.accounts.fanout.total_staked_shares;
  let shares_diff = ctx.accounts.fanout.total_shares
      .checked_sub(tss)
      .unwrap();
  ...
  let mut dist_amount_u64: u64 = dist_amount.checked_div(TWELVE_PREC).unwrap().try_into().unwrap();

  // Account for dust
  let dust: u64 = dist_amount.checked_sub(u128::from(dist_amount_u64).checked_mul(TWELVE_PREC).unwrap()).unwrap().try_into().unwrap();

  let new_dust = dust + ctx.accounts.voucher.total_dust;
  let whole_dust = dust.checked_div(u64::try_from(TWELVE_PREC).unwrap()).unwrap();
  if whole_dust > 1 {
    dist_amount_u64 += 1;
    ctx.accounts.voucher.total_dust = new_dust.checked_sub(u64::try_from(TWELVE_PREC).unwrap()).unwrap();
  } else {
    ctx.accounts.voucher.total_dust = new_dust;
  }

  let signer_seeds: &[&[u8]] = voucher_seeds!(ctx.accounts.voucher);
  token::transfer(ctx.accounts.transfer_ctx().with_signer(&[signer_seeds]), dist_amount_u64)?;

  ctx.accounts.voucher.total_inflow = ctx.accounts.fanout.total_inflow;
  ctx.accounts.fanout.last_snapshot_amount = curr_balance;
  ctx.accounts.fanout.total_inflow = ctx.accounts.fanout.total_inflow.checked_add(inflow).unwrap().checked_add(unstaked_correction).unwrap();
