...
#[derive(Accounts)]
pub struct InitializePositionV1<'info> {
  #[account(
    mut,
    has_one = collection
  )]
...
impl<'info> InitializePositionV1<'info> {
...
pub fn handler(ctx: Context<InitializePositionV1>, args: InitializePositionArgsV0) -> Result<()> {
  // Load accounts.
