...
async fn test_deposit_cliff_minimum_lockup() -> Result<(), TransportError> {
  ...
  assert_eq!(after_reset.voter_weight, after_reset.vault); // minimum_saturated locking bonus
  ...
  assert_eq!(after_cliff.voter_weight, 0); // zero vote weight since nothing is locked anymore
  assert_eq!(after_cliff.vault, 9000);
  assert_eq!(after_cliff.deposit, 9000);

  withdraw(9001).await.expect_err("withdrew too much");
  withdraw(9000).await.unwrap();
