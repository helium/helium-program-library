...
pub fn schedule_impl(ctx: &mut ScheduleTaskV0, args: ScheduleTaskArgsV0) -> Result<()> {
  ...
  // HNT always runs after the DC leg of the same slot.
  let next_hnt_time = next_time + if TESTING { 0 } else { 5 * 60 };
  auto_top_off.next_task = ctx.task.key();
  auto_top_off.next_hnt_task = ctx.hnt_task.key();
  auto_top_off.next_task_time = next_time;
  auto_top_off.next_hnt_task_time = next_hnt_time;
  ...
  queue_task_v0(
    ...
    QueueTaskArgsV0 {
      trigger: TriggerV0::Timestamp(next_hnt_time),
