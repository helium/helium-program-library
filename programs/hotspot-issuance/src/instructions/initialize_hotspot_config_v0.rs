...
use mpl_token_metadata::{instruction::{create_metadata_accounts_v3, create_master_edition_v3}, state::CollectionDetails, ID as TOKEN_METADATA_ID};
use crate::state::*;
...
pub struct InitializeHotspotConfigV0<'info> {
  ...
  #[account(
    ...
    seeds = [hotspot_config.key().as_ref(), token_program.key().as_ref(), collection.key().as_ref()],
    bump
  )]
