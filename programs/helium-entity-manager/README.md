...
Release / upgrade: add a program changeset that names `helium-entity-manager` in [`.changeset-programs/`](../../.changeset-programs), and bump `version` in `programs/helium-entity-manager/Cargo.toml`. After the change reaches `master`, push the `program-helium-entity-manager-<version>` tag by hand. The tag's run opens the Squads proposal. See [CI / deployment overview](../../README.md#ci--deployment-overview).
