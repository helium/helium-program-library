...
pub struct TransferV0<'info> {
  ...
  /// CHECK: This account needs to be closed. You can't transfer while enrolled
  #[account(
    seeds = ["enrolled_position".as_bytes(), source_position.key().as_ref()],
    bump,
    constraint = source_enrolled_position.data_is_empty() && source_enrolled_position.lamports() == 0 @ ErrorCode::PositionChangeWhileEnrolled
  )]
  pub source_enrolled_position: UncheckedAccount<'info>,
  ...
  /// CHECK: This account needs to be closed. You can't transfer while enrolled
  #[account(
    seeds = ["enrolled_position".as_bytes(), target_position.key().as_ref()],
    bump,
    constraint = target_enrolled_position.data_is_empty() && target_enrolled_position.lamports() == 0 @ ErrorCode::PositionChangeWhileEnrolled
  )]
  pub target_enrolled_position: UncheckedAccount<'info>,
  ...
  transfer_v0(
    CpiContext::new_with_signer(
      ...
      VsrTransferV0 {
        ...
        position_update_authority: ctx.accounts.vetoken_tracker.to_account_info(),
      ...
      &[&[
        b"vetoken_tracker",
        ctx.accounts.vetoken_tracker.registrar.key().as_ref(),
        &[ctx.accounts.vetoken_tracker.bump_seed],
      ]],
    ),
    VsrTransferArgsV0 {
      amount: args.amount,
