...
pub struct ResetLockupV0<'info> {
  ...
  /// CHECK: This account needs to be closed. You can't reset_lockup while enrolled
  #[account(
    seeds = ["enrolled_position".as_bytes(), position.key().as_ref()],
    bump,
    constraint = enrolled_position.data_is_empty() && enrolled_position.lamports() == 0 @ ErrorCode::PositionChangeWhileEnrolled
  )]
  pub enrolled_position: UncheckedAccount<'info>,
