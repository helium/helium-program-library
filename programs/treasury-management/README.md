...
Release / upgrade: add a program changeset that names `treasury-management` in [`.changeset-programs/`](../../.changeset-programs), and bump `version` in `programs/treasury-management/Cargo.toml`. After the change reaches `master`, push the `program-treasury-management-<version>` tag by hand. The tag's run opens the Squads proposal. See [CI / deployment overview](../../README.md#ci--deployment-overview).
