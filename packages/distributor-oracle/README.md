...
Deploy: the service auto-tag bot pushes `docker-oracle-distributor-oracle-<version>` on each merge to `develop` that changes it. Push the tag by hand for a hotfix, a minor, or a major (see [repo README](../../README.md#deploying-docker-services)). The k8s bump stays manual.
