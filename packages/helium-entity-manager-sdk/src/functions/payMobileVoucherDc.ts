...
export async function payMobileVoucherDc({
  program,
  ceProgram,
  ...
  const dcFeeUsd = toNumber(fees.dcOnboardingFee, 5);

  const quoteResponse = await (
    await fetch(
      `${JUPITER_URL}/quote?inputMint=${USDC_MINT.toBase58()}&outputMint=${HNT_MINT.toBase58()}&amount=${toBN(
        dcFeeUsd,
        6
