...
import { init, lazyDistributorKey } from "@helium-foundation/lazy-distributor-sdk";
...
export async function getPendingRewards(program: Program<LazyDistributor>, mint: PublicKey) {
  const lazyDistributor = await program.account.lazyDistributorV0.fetch(LAZY_KEY);

  const [recipient] = recipientKey(LAZY_KEY, mint);
  const maybeRecipient = await program.account.recipientV0.fetchNullable(recipient);
  ...
  const sortedRewards = ((maybeRecipient?.currentRewards as (BN | null)[]) || [])
    .sort((a, b) => (a || new BN(0)).sub((b || new BN(0))).toNumber());
  const sortedOracleRewards = oracleRewards.map(rew => rew.currentRewards)
    .sort((a, b) => new BN(a).sub(new BN(b)).toNumber());

  let median = sortedRewards[Math.floor(sortedRewards.length / 2)];
  let oracleMedian = new BN(sortedOracleRewards[Math.floor(sortedOracleRewards.length / 2)]);
