...
export async function run(args: any = process.argv) {
  ...
  const delegatedPositions =
    await hsdProgram.account.delegatedPositionV0.fetchMultiple(
      bots.map((bot) => bot.account.delegatedPosition)
    );
  const positions = await hsdProgram.account.positionV0.fetchMultiple(
    delegatedPositions.map((dp) => dp?.position || PublicKey.default)
  );
