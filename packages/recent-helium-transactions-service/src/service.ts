...
export class RecentTransactionsService {
  ...
  private async processTransaction(signature: string, programId: string) {
    try {
      ...
      const programGroups = new Map<
        string,
        { route: string; transactions: TransactionDefinition[] }
      >();
      ...
      for (const transaction of group.transactions) {
        try {
          const parsedInstructions = await this.parser.parseTransaction(
            tx,
            transaction
          );
          ...
          logger.error(
            `Error parsing transaction for ${transaction.idlName}:`,
            error
          );
        }
      }
  ...
  getTransactions(
    ...
    const transactions = this.bufferManager.getTransactions(
      route,
      args.untilBlock,
      args.limit
    );
    if (args.types && args.types.length > 0) {
      return transactions.filter(
        (tx) => !args.types || args.types.includes(tx.name)
      );
    }
    return transactions;
  }
