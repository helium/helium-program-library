...
export class TransactionParser {
  ...
  private isTokenTransferInstruction(
  ...
  private async extractAssetNameFieldValue(
    ...
    const asset = await getAsset(
      process.env.ASSET_API_URL || this.connection.rpcEndpoint,
      assetId
    );
  ...
  private async extractRecipientAssetNameFieldValue(
    ...
    const program = await init(
      new AnchorProvider(this.connection, {} as Wallet, {
        commitment: "confirmed",
      }),
      PROGRAM_ID,
      await this.loadIdl(PROGRAM_ID)
    );
  ...
  private async extractFieldValue(
    ...
    switch (source.type) {
      ...
      case "innerTokenTransfer":
        return this.extractTokenTransferFieldValue(
          source,
          instruction,
          innerInstructions
        );
  ...
  async parseTransaction(
    ...
    try {
      ...
      const matchingInstructions = txWithMeta.transaction.message.instructions
        ...
        .filter(
          ({ ix }) => "programId" in ix && ix.programId.equals(programId)
        );
      ...
      const parsedInstructions = (
        await Promise.all(
          matchingInstructions.map(async ({ ix, index }) => {
            if (!("data" in ix)) return null;

            const decodedInstruction = coder.decode(ix.data, "base58");
            if (
              !decodedInstruction ||
              decodedInstruction.name !== definition.idlName
            ) {
            ...
            const idlInstruction = idl.instructions?.find(
              (inst) => inst.name === decodedInstruction.name
            );
            if (!idlInstruction) {
              logger.debug({
            ...
            const formattedAccounts = this.formatAccountsRecursively(
              idlInstruction.accounts,
              ix.accounts
            );
            const innerInstructions =
              txWithMeta.meta?.innerInstructions?.find(
