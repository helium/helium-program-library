...
export class TransactionCache {
  ...
  constructor(maxSize = 10000, maxAgeMs = 60 * 60 * 1000) {
    // Default 10k entries, 1 hour max age
    this.cache = new Map();
  ...
  cleanup(): void {
    ...
    if (entriesRemoved > 0) {
      logger.info(
        `Cleaned up ${entriesRemoved} entries from transaction cache. New size: ${this.cache.size}`
      );
    }
  }
