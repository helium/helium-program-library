...
async function run() {
  ...
  const oracleIndex = priceOracleAcc.oracles.findIndex((x) => x.authority.equals(provider.wallet.publicKey));
  if (oracleIndex == -1) {
    throw new Error("Couldn't find your wallet as an oracle authority on this price oracle");
  }
