...
server.get<{ Params: { mintKey: string } }>(
  ...
  async (request, reply) => {
    ...
    const decimals = (await getMint(provider.connection, votingMint.mint))
      .decimals;
    ...
    return {
      name,
      description: `Voting Escrow Token Position of ${uiAmount} tokens${
        kind === "constant"
          ? "."
          : ` locked until ${lockEndDate} with kind ${kind}`
      }`,
      image:
      ...
      attributes: [
        ...
        {
          trait_type: "amount_deposited_native",
          value: positionAcc.amountDepositedNative.toString(),
        },
        { trait_type: "amount_deposited", value: uiAmount },
        {
          trait_type: "voting_mint_config_idx",
          value: positionAcc.votingMintConfigIdx,
        },
        { trait_type: "voting_mint", value: votingMint.mint.toBase58() },
        {
          trait_type: "start_ts",
          value: positionAcc.lockup.startTs.toString(),
        },
