...
import {
  PROGRAM_ID as MINI_FANOUT_PROGRAM_ID,
  miniFanoutKey,
  queueAuthorityKey,
} from "@helium/mini-fanout-sdk";
...
export async function claimWelcomePack({
  ...
  return program.methods
    ...
    .accountsStrict({
      ...
      welcomePack,
      assetReturnAddress: welcomePackAcc.assetReturnAddress.equals(
        PublicKey.default
      )
        ? claimer
        : welcomePackAcc.assetReturnAddress,
      recipient: recipientKey(welcomePackAcc.lazyDistributor, assetId)[0],
      rewardsRecipient:
        welcomePackAcc.rewardsSplit.length > 1
          ? miniFanout
          : welcomePackAcc.rewardsSplit[0].wallet.equals(PublicKey.default)
          ? claimer
          : welcomePackAcc.rewardsSplit[0].wallet,
