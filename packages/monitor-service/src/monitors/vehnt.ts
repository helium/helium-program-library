...
WITH
  readable_positions AS (
    SELECT p.*,
      ...
      CASE WHEN floor(p.genesis_end / (60 * 60 * 24)) * (60 * 60 * 24) > current_ts THEN cast(r.voting_mints[p.voting_mint_config_idx + 1]->>'genesisVotePowerMultiplier' as numeric) ELSE 1 END as genesis_multiplier,      GREATEST(
        cast(
          p.end_ts - 
