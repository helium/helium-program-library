...
../anchor/target/debug/anchor idl upgrade hemABtqNUst4MmqsVcuN217ZzBspENbGt9uueSe5jts --filepath target/idl/helium_entity_manager.json --provider.cluster devnet --provider.wallet ~/.config/solana/id.json
...
../anchor/target/debug/anchor idl upgrade 1atNarMiQ8RMLkcTwqHHUESs2f4SB3uouPKFLbXcMwE --filepath target/idl/lazy_transactions.json --provider.cluster devnet --provider.wallet ~/.config/solana/id.json
../anchor/target/debug/anchor idl upgrade hvsrY9UBtHhYRvstM2BWCsni81kevfn7B2DEhYbGA1a --filepath target/idl/voter_stake_registry.json --provider.cluster devnet --provider.wallet ~/.config/solana/id.json
