...
fn bound_by_account_sink_cursor(
  ...
  Some(std::cmp::min(query_max_block, cursor_block))
}
...
impl DatabaseClient {
  ...
  async fn execute_query_inner(
    ...
    if job.query_name == "construct_atomic_hotspots" {
      ...
      Self::consume_stream(&mut stream, &mut rows, &job.name).await?;
    } else {
      info!(
        "Querying job '{}' with query '{}', processing blocks > {} to {} ({} blocks)",
        job.name, job.query_name, last_block_display, target_block, blocks_span
      );
  ...
  async fn consume_stream(
    stream: &mut (impl futures::Stream<Item = Result<sqlx::postgres::PgRow, sqlx::Error>> + Unpin),
    ...
    while let Some(row_result) = stream.next().await {
      ...
      if row_count % 10_000 == 0 {
        debug!("Streamed {} rows for job '{}'", row_count, job_name);
      }
    }

    if row_count > 0 {
      debug!("Streamed {} total rows for job '{}'", row_count, job_name);
    }
    Ok(())
  }
  ...
  pub async fn mark_processed(&self, changes: &[ChangeRecord], target_block: u64) -> Result<()> {
    ...
    let mut processed_jobs = std::collections::HashSet::new();
    ...
    for change in changes {
      processed_jobs.insert(change.job_name.clone());
    }
...
mod tests {
  ...
  async fn last_processed_block(pool: &PgPool, job_name: &str) -> Option<i64> {
    sqlx::query("SELECT last_processed_block FROM atomic_data_polling_state WHERE job_name = $1")
      .bind(job_name)
      .fetch_one(pool)
      .await
      .expect("read polling state")
      .get("last_processed_block")
  }

  #[sqlx::test(migrations = "tests/migrations")]
  async fn ownership_waits_for_key_to_asset_behind_account_sink_cursor(pool: PgPool) {
    ...
    // asset_ownership landed block 200; account_sink has only committed up to 199.
    sqlx::query("INSERT INTO asset_owners (asset, owner, last_block) VALUES ('asset_race', 'wallet_race', 200)")
      .execute(&pool)
      .await
      .unwrap();
    ...
    sqlx::query(
      "INSERT INTO key_to_assets (address, entity_key, asset, key_serialization) \
       VALUES ('kta_race', '\\xabcdef', 'asset_race', '\"b58\"'::jsonb)",
    )
    .execute(&pool)
    .await
    .unwrap();
