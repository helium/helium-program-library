...
const WP_OWNERSHIP_SQL: &str = include_str!(concat!(
  env!("CARGO_MANIFEST_DIR"),
  "/src/sql/construct_welcome_pack_ownership.sql"
));

const WP_REWARD_SQL: &str = include_str!(concat!(
  env!("CARGO_MANIFEST_DIR"),
  "/src/sql/construct_welcome_pack_reward_destination.sql"
));
...
pub async fn seed_asset_owner(pool: &PgPool, asset: &str, owner: &str, last_block: i64) {
  sqlx::query("INSERT INTO asset_owners (asset, owner, last_block) VALUES ($1, $2, $3)")
    .bind(asset)
    .bind(owner)
    .bind(last_block)
    .execute(pool)
...
pub async fn seed_welcome_pack(pool: &PgPool, address: &str, owner: &str, last_block: i64) {
  sqlx::query("INSERT INTO welcome_packs (address, owner, last_block) VALUES ($1, $2, $3)")
    .bind(address)
    .bind(owner)
    .bind(last_block)
    .execute(pool)
...
pub async fn run_wp_ownership_query(
  pool: &PgPool,
  last_processed_block: i64,
  max_block: i64,
) -> Vec<Value> {
